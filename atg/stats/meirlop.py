import sys
import os
import os.path
import pickle

from meirlop import analyze_scored_fasta_data_with_lr, read_scored_fasta, read_motif_matrices

def run_meirlop(args):
    scored_fasta_file = args.scored_fasta_file
    motif_matrix_file = args.motif_matrix_file
    save_scan = args.save_scan
    max_k = args.max_k
    n_jobs = args.jobs
    
    os.environ['OMP_NUM_THREADS'] = f'{n_jobs}'
    os.environ['MKL_NUM_THREADS'] = f'{n_jobs}'
    
    sequence_dict, score_dict = read_scored_fasta(scored_fasta_file)[:-1]
    motif_matrix_dict = read_motif_matrices(motif_matrix_file)[0]
    
    (lr_results_df, 
     lr_input_df, 
     motif_peak_set_dict, 
     scan_results_df) = analyze_scored_fasta_data_with_lr(
        sequence_dict, 
        score_dict, 
        motif_matrix_dict, 
        max_k = max_k, 
        n_jobs = n_jobs)
    
    outpath = os.path.normpath(args.output_dir)
    if not os.path.exists(outpath):
        os.makedirs(outpath)
    
    outpath_lr_results = os.path.normpath(args.output_dir + '/lr_results.tsv')
    outpath_lr_input = os.path.normpath(args.output_dir + '/lr_input.tsv')
    outpath_motif_peak_set_dict = os.path.normpath(args.output_dir + '/motif_peak_set_dict.p')
    outpath_scan_results = os.path.normpath(args.output_dir + '/scan_results.tsv')
    
    lr_results_df.to_csv(outpath_lr_results, sep = '\t', index = False)
    lr_input_df.to_csv(outpath_lr_input, sep = '\t', index = False)
    if save_scan:
        scan_results_df.to_csv(outpath_scan_results, sep = '\t', index = False)
    pickle.dump(motif_peak_set_dict, open(outpath_motif_peak_set_dict, 'wb'))

def setup_subparsers(subparsers):
    description = ('Determine enrichment '
                   'of motifs in a list of '
                   'scored sequences.')
    
    meirlop_parser = subparsers.add_parser('meirlop', help = description)
        parser.add_argument('scored_fasta_file', 
                        metavar = 'scored_fasta_file',
                        type = argparse.FileType('r'), 
                        help = (
                            'A scored fasta file, '
                            'where sequence headers are of form: \n'
                            '>sequence_name sequence_score'
                        ))
    
       meirlop_parser.add_argument('motif_matrix_file', 
                        metavar = 'motif_matrix_file', 
                        type = argparse.FileType('r'), 
                        help = (
                            'A motif matrix file '
                            'in JASPAR format.'
                        ))
    
    meirlop_parser.add_argument('output_dir', 
                        metavar = 'output_dir', 
                        type = str, 
                        help = (
                            'Create this directory and write output to it.'
                        ))
    
    meirlop_parser.add_argument('--jobs', 
                        metavar = 'jobs', 
                        dest = 'jobs', 
                        type = int, 
                        default = 1, 
                        help = ( 
                            'The number of jobs for multithreading. \n'
                            'Note: statsmodels may exceed this during '
                            'logistic regression.'
                        ))
    
    meirlop_parser.add_argument('--scan', 
                        dest = 'save_scan', 
                        action='store_true', 
                        help = (
                            'Set this flag to write motif ' 
                            'scanning results table to ' 
                            'output directory.'
                        ))
    
    meirlop_parser.add_argument('--kmer', 
                        metavar = 'max_k', 
                        dest = 'max_k', 
                        type = int, 
                        default = 2, 
                        help = (
                            'Set maximum length of kmers to '
                            'consider during regression. '
                            'Frequency of kmers will be used '
                            'as a covariate in logistic regression. '
                            'Default = 2'
                        ))
    
    enrichment_parser.set_defaults(func=run_meirlop)
