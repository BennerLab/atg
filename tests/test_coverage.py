...
RNASEQ_FILE = os.path.join(os.path.dirname(__file__), 'data', 'rnaseq.bam')


class CoverageCalculatorTest(unittest.TestCase):
    def setUp(self):
        self.coverage_calculator = coverage.CoverageCalculator(RNASEQ_FILE)

    def test_read_count(self):
        """
        test file contains 198 uniquely mapped R1 reads
        """
        self.assertEqual(self.coverage_calculator.mapped_read_count, 198)
