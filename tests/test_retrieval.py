    ...
    def test_url_fetch(self):
        with tempfile.TemporaryDirectory() as working_directory:
            ...
            with open(assembly_filename, 'r') as assembly:
                ...
                assert firstline[0] == 'chr1'
                assert firstline[0] != 'chr2'
                assert len(firstline) == ASSEMBLY_FILE_ENTRIES_PER_LINE
...
class TestATGDataTracker:
    ...
    def test_yeast_retrieval(self):
        ...
        assert 'yeast' in os.listdir(self.temp_dir.name)
...
class TestEnsemblGenomes:
    ...
    def test_ensembl_table_read(self):
        corn_record = self.ensembl_genomes.ensembl_species_df.loc[self.ensembl_genomes.ensembl_species_df['species'] ==
                                                                  'zea_mays']
        assert corn_record.shape[0] == 1
        assert corn_record.iloc[0]['division'] == 'EnsemblPlants'

    def test_species_information(self):
        assert self.ensembl_genomes.get_species_information('zea_mays') == self.corn_information
        assert self.ensembl_genomes.get_species_information('apis_mellifera') == self.bee_information
        assert self.ensembl_genomes.get_species_information('agaricus_bisporus_var_bisporus_h97') == \
    ...
    def test_collect_species(self):
        ...
        assert species_df.equals(self.ensembl_genomes.collect_species_information(species_list))

    def test_retrieval(self):
        assert not self.ensembl_genomes.retrieve_species_data('homo_sapiens')
        assert self.ensembl_genomes.retrieve_species_data('agaricus_bisporus_var_bisporus_h97')
